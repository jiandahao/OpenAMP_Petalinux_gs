...
{
	/*CPU不能直接读写内存，而是通过cache来访问内存(这是物理结构决定的)，由于remote端直接将数据写入内存中
	为了使master core读取正确的数据，应该先更新cache，将内存中的数据反映到cache上*/
	flush_cache_all();
	outer_flush_range(virtio_info->mem_start, virtio_info->mem_end);//clean and invalidate outer cache lines
	if (vring_interrupt(0, virtio_info->vrings[0].vq) == IRQ_NONE)
		//通过调用vring_interrupt(defined in virtio_ring.c)去读取共享内存中的数据，该函数实际通过调用vqs->callback
		//发送/接收队列的回调callback接口在virtio_rpmsg_bus.c中定义，于rpmsg_probe()中完成队列和回调的绑定
...
{/*send virtqueue的notify接口，用于向remote端发送中断通知*/
...
static int rpmsg_virtio_find_vqs(struct virtio_device *vdev,
					...
					const char *names[])
{//寻找存在的virtqueue，由于在该设计中一开始是不存在任何virtqueue的，所以在这里调用
	//vring_new_virtqueue创建新的virtqueue
		int i,size;
		struct rpmsg_vring *local_vring;
		for(i = 0;i < nvqs ; i++)//nvqs值在virtio driver的probe函数中指定为2
		{
			...
			local_vring->vq = vring_new_virtqueue(i,//virtqueue由desc table,avail ring和used ring 三部分组成
...
static const struct virtio_config_ops rpmsg_virtio_config_ops = {
	...
	.find_vqs	= rpmsg_virtio_find_vqs,//virtio device和virtio driver匹配成功后会调用，，，within rpmsg_probe function(virtio_rpmsg_bus.c)
	.del_vqs	= rpmsg_virtio_del_vqs,
	.reset		= rpmsg_virtio_reset,
	//virtio device和virtio driver匹配成功后会调用,由kick时调用，，，within rpmsg_probe function(virtio_rpmsg_bus.c)
...
static int rpmsg_virtio_probe(struct platform_device *pdev){
	...
	virtio_info->mem_start = 0x08000000;//定义共享内存区域，地址为物理地址
	virtio_info->mem_end = 0x10000000;
	virtio_info->num_descs = 256;//定义描述符表的表项个数为256个
	virtio_info->vring_align = 0x1000;//4k对齐
	virtio_info->interrupt_vring0 = 15;//软中断号，master接收到该中断后执行数据接收
	virtio_info->interrupt_vring1 = 14;//软中断号，master往remote发送该中断通知其接收数据
	virtio_info->num_vrings = 2;//virtqueue的个数，一个用于收一个用于发。
	...
	ret = dma_declare_coherent_memory(&pdev->dev,//分配物理上连续的一段一致性内存
					virtio_info->mem_start,//分配的内存区域为mem_start~mem_end
	...
	ret = dma_set_coherent_mask(&pdev->dev, DMA_BIT_MASK(32));//限制内存寻址范围
	if (ret) {
		dev_err(&pdev->dev, "dma_set_coherent_mask: %d\n", ret);
		return -ENODEV;
	}
	INIT_WORK(&workqueue,handle_event);//初始化工作队列
	...
	ret = register_virtio_device(virtio_dev);//注册virtio设备，该注册接口可见virtio.c
