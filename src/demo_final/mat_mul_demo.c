...
int exect_shell_command(const char *argv[]){
	pid_t pid;
	pid = fork();
	switch(pid){
		case -1:
...
char *const load_vritio_device_driver[] = {
	"modprobe","virtio_device_driver",0
};
char *const load_rpmsg_user_dev_driver[] = {
	"modprobe","rpmsg_user_dev_driver",0
};
char *const realse_vritio_device_driver[] = {
	"modprobe","virtio_device_driver","-r",0
};
char *const realse_rpmsg_user_dev_driver[] = {
	"modprobe","rpmsg_user_dev_driver","-r",0
};
int main(int argc, char *argv[])
{	
	// if(exect_shell_command(load_vritio_device_driver))
	// {
	// 	 printf("Loaded virtio_device_driver module done\n");
	// 	if(exect_shell_command(load_rpmsg_user_dev_driver))
	// 	{
			//child_pid = wait(&stat_val);	
			printf("Loaded rpmsg_user_dev_driver module done\n");			
			...
			if(fd < 0){
				printf("\r\n  rpmsg dev is not ready ,please try again later \r\n");
				return 0;
			}
			...
			// exect_shell_command(realse_vritio_device_driver);
			// exect_shell_command(realse_rpmsg_user_dev_driver);
	// 	}
	// }
